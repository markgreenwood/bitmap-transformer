...
Bitmap.prototype.transform = function (/* xfmrFunc */) {
  // TODO: fill in function
};

Bitmap.prototype.redderImage = function() {
  for (let i = 0; i < this.color_table.length; i++) {
    this.color_table[i].red = Math.min(this.color_table[i].red * 2, 255);
  }
};

Bitmap.prototype.greenerImage = function() {
  for (let i = 0; i < this.color_table.length; i++) {
    this.color_table[i].green = Math.min(this.color_table[i].green * 2, 255);
  }
};

Bitmap.prototype.bluerImage = function() {
  for (let i = 0; i < this.color_table.length; i++) {
    this.color_table[i].blue = Math.min(this.color_table[i].blue * 2, 255);
  }
};
...
Bitmap.prototype.updateColorTableInBuffer = function() {
  let color_tbl_offs = 14 + this.dib_header.dib_hdr_size;
  for (let i = 0; i < this.color_table.length; i++) {
