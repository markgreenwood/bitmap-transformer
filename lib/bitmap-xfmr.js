...
function Bitmap(filename) {
  ...
  if (this.dib_header.bits_per_pixel === 8) this.palette = true;
  else this.palette = false;
  // or...
  // this.palette = (this.dib_header.bits_per_pixel === 8);
  ...
  this.imageData = [];
  for (let row = 0; row < this.dib_header.img_height; row++) {
    for (let col = 0; col < this.dib_header.img_width; col++) {
      if (this.palette === 'palette') {
        this.imageData.push(new PalettePixel(this.buffer.readUInt8(pixel_idx)));
        pixel_idx += 1;
      }
      else {
        let b = this.buffer.readUInt8(pixel_idx);
        let g = this.buffer.readUInt8(pixel_idx + 1);
        let r = this.buffer.readUInt8(pixel_idx + 2);
        this.imageData.push(new NonpalettePixel(r, g, b));
        pixel_idx += 3;
      }
    }
  }
